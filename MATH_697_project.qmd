...
where $\beta = (0.8, 0, 0, 1, 0, 0, 0.6, 0)^T$ and $x_i$'s are marginally standard normal with correlation $\rho=0.5$, Furthermore, censoring times are exponentially distributed with mean $U\text{exp}(\mathbf{x^T\beta_0})$, $U\sim \text{Uniform}(1,3)$. Our variable selection procedures are compared to the maxium partial likelihood estimated model using the Median of Relative Model Errors (MRME), the ratio of of the model errors defined in $\eqref{modeler}$. Additionally we record the average number of correct and incorrect coefficients estimated to be 0. The results of these simulations can be seen in @tbl-two. Standard deviations of regression coefficients can be seen in @tbl-three for each variable selection technique as well.
...
```{r}
#| label: tbl-three
kable(data2, col.names = c("Method", "Beta1 SD", "Beta4 SD", "Beta7 SD"),caption="test")
```
...
Fitting a full Cox Proportional Hazards model to the data yields the following results:

Look at @tbl-five and @tbl-six

Next we consider using our various variable selection methods and show the results:


Note tha diagnostic plots are avaiable in @sec-app.
...
kable(cox_summary_all,caption="Test")
```
...
load("./data/best_model_AIC.RData")
load("./data/best_model_BIC.RData")
