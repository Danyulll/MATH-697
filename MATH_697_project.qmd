...
@tbl-five displays the results of fitting a Cox Proportional Hazards model to the dataset. We see that all variables, excluding sex, are considered to be statistically significant. There seems to be some debate as to whether or not sex is an important variable in PBC outcomes. Studies such as [@smyk2012sex] and [@lleo2016evolving] find that PBC is far more rare in men and presents with some different symptoms. Specifically, [@lleo2016evolving] discusses that the historical consensus has been that sex does not affect survival times for PBC, but there is new data to suggest otherwise. Note that diagnostic plots of the proportional hazards assumption, and any discussion surrounding them, are available in @sec-app.
