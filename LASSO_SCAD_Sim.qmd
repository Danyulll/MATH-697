...
bic_simulation <- function(N, n) {
  ...
  p <- length(Beta) 
  Beta_part_all = matrix(NA, N, p)
  Beta_AIC_all = matrix(NA, N, p)
  Beta_BIC_all = matrix(NA, N, p)
  ME_part = rep(NA, N)
  ...
  while (t <= N) {
    ...
    if (is.finite(AICs[best_AIC_idx])) {
      ...
      # Store the coefficients in Beta_AIC_all
      Beta_AIC_all[t, best_fit_AIC_indices[[t]]] <- coef(best_fit_AIC[[t]])
    }
    ...
    if (is.finite(BICs[best_BIC_idx])) {
      ...
      # Store the coefficients in Beta_BIC_all
      Beta_BIC_all[t, best_fit_BIC_indices[[t]]] <- coef(best_fit_BIC[[t]])
    }

    t <- t + 1
  }

  # Calculate mean and standard deviation for beta_1, beta_4, and beta_7 for AIC and BIC
  beta_1_stats_AIC <- c(mean = mean(Beta_AIC_all[, 1], na.rm = TRUE), sd = sd(Beta_AIC_all[, 1], na.rm = TRUE))
  beta_4_stats_AIC <- c(mean = mean(Beta_AIC_all[, 4], na.rm = TRUE), sd = sd(Beta_AIC_all[, 4], na.rm = TRUE))
  beta_7_stats_AIC <- c(mean = mean(Beta_AIC_all[, 7], na.rm = TRUE), sd = sd(Beta_AIC_all[, 7], na.rm = TRUE))
  
  beta_1_stats_BIC <- c(mean = mean(Beta_BIC_all[, 1], na.rm = TRUE), sd = sd(Beta_BIC_all[, 1], na.rm = TRUE))
  beta_4_stats_BIC <- c(mean = mean(Beta_BIC_all[, 4], na.rm = TRUE), sd = sd(Beta_BIC_all[, 4], na.rm = TRUE))
  beta_7_stats_BIC <- c(mean = mean(Beta_BIC_all[, 7], na.rm = TRUE), sd = sd(Beta_BIC_all[, 7], na.rm = TRUE))
...
data <- data.frame(
  ...
  Column2 = c("",round(lasso_scad_cox_sim_75.out$MRME_SCAD,2) ,
              ...
              round(aic_bic_simulation_75.out$tb3_AIC_BIC$MRME_AIC,2),
              round(aic_bic_simulation_75.out$tb3_AIC_BIC$MRME_BIC,2),
              "",
              round(lasso_scad_cox_sim_100.out$MRME_SCAD,2),
              round(lasso_scad_cox_sim_100.out$MRME_LASSO,2),
              round(aic_bic_simulation_100.out$tb3_AIC_BIC$MRME_AIC,2),
              round(aic_bic_simulation_100.out$tb3_AIC_BIC$MRME_BIC,2)),
  Column3 = c("",
              ...
              round(aic_bic_simulation_75.out$tb2_AIC_BIC$zero_coeff_AIC,2),
              round(aic_bic_simulation_75.out$tb2_AIC_BIC$zero_coeff_BIC,2),
              "",
              ...
              round(aic_bic_simulation_100.out$tb2_AIC_BIC$zero_coeff_AIC,2),
              round(aic_bic_simulation_100.out$tb2_AIC_BIC$zero_coeff_BIC,2)),
  Column4 = c("",
              ...
              round(aic_bic_simulation_75.out$tb2_AIC_BIC$incorrect_nonzero_AIC,2),
              round(aic_bic_simulation_75.out$tb2_AIC_BIC$incorrect_nonzero_BIC,2),
              "",
              ...
              round(aic_bic_simulation_100.out$tb2_AIC_BIC$incorrect_nonzero_AIC,2),
              round(aic_bic_simulation_100.out$tb2_AIC_BIC$incorrect_nonzero_BIC,2))
)
...
data <- data.frame(
  Column1 = c("**n=75**","SCAD","LASSO","AIC/BIC","**n=100**","SCAD","LASSO","AIC/BIC"),
  Column2 = c("",round(lasso_scad_cox_sim_75.out$Simulate_Beta_SCAD_std[1],2) ,
              round(lasso_scad_cox_sim_75.out$Simulate_Beta_LASSO_std[1],2),
              round(aic_bic_simulation_75.out$beta_stats$beta_1_stats[2],2),
              "",
              round(lasso_scad_cox_sim_100.out$Simulate_Beta_SCAD_std[1],2),
              round(lasso_scad_cox_sim_100.out$Simulate_Beta_LASSO_std[1],2),
              round(aic_bic_simulation_100.out$beta_stats$beta_1_stats[2],2)),
  Column3 = c("",
              round(lasso_scad_cox_sim_75.out$Simulate_Beta_SCAD_std[4],2) ,
              round(lasso_scad_cox_sim_75.out$Simulate_Beta_LASSO_std[4],2),
              round(aic_bic_simulation_75.out$beta_stats$beta_4_stats[2],2),
              "",
              round(lasso_scad_cox_sim_100.out$Simulate_Beta_SCAD_std[4],2),
              round(lasso_scad_cox_sim_100.out$Simulate_Beta_LASSO_std[4],2),
              round(aic_bic_simulation_100.out$beta_stats$beta_4_stats[2],2)),
  Column4 = c("",round(lasso_scad_cox_sim_75.out$Simulate_Beta_SCAD_std[7],2) ,
              round(lasso_scad_cox_sim_75.out$Simulate_Beta_LASSO_std[7],2),
              round(aic_bic_simulation_75.out$beta_stats$beta_7_stats[2],2),
              "",
              round(lasso_scad_cox_sim_100.out$Simulate_Beta_SCAD_std[7],2),
              round(lasso_scad_cox_sim_100.out$Simulate_Beta_LASSO_std[7],2),
